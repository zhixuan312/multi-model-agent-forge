...
describe('reads + toggles', () => {
  it('listLoops returns rows; getLoop returns a row or null', async () => {
    expect(await listLoops({ db: createMockDb({ 'select:loop_def': [loopRow(), loopRow({ id: 'l2' })] }) })).toHaveLength(2);
    expect(await getLoop('loop-1', { db: createMockDb({ 'select:loop_def': [loopRow()] }) })).not.toBeNull();
    expect(await getLoop('x', { db: createMockDb({ 'select:loop_def': [] }) })).toBeNull();
  });

  it('deleteLoop / setLoopEnabled report deleted|updated vs not_found', async () => {
    expect((await deleteLoop('loop-1', { db: createMockDb({ 'delete:loop_def': [{ id: 'loop-1' }] }) })).kind).toBe('deleted');
    expect((await deleteLoop('x', { db: createMockDb({ 'delete:loop_def': [] }) })).kind).toBe('not_found');
    expect((await setLoopEnabled('loop-1', false, { db: createMockDb({ 'update:loop_def': [{ id: 'loop-1' }] }) })).kind).toBe('updated');
    expect((await setLoopEnabled('x', false, { db: createMockDb({ 'update:loop_def': [] }) })).kind).toBe('not_found');
  });
});
