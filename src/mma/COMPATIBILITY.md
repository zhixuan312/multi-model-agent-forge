...
|---|---|
| **`DELETE /task/:taskId`** — cooperative cancellation | Adopted, one path end to end: **Stop & take over** → `take_over` → `cancelInFlightBatches` → `PollManager.requestCancel()` (idempotent) → `MmaClient.cancel()`. Cancellation is REQUESTED, not immediate: the poll loop carries each batch to its terminal `cancelled` envelope, and the overlay holds a `stopping` state — driven by the `cancelling` phase on `task.progress`/`dispatch.progress` plus the dispatch terminals — until it does, so the UI never claims a stop the engine has not made. A per-batch `POST /api/projects/[id]/batches/[batchId]/cancel` route also existed, callerless, from this adoption until `2026-08-04`; it was deleted rather than left as a second way to do the same thing (`git log -- 'app/api/projects/[id]/batches'`) |
