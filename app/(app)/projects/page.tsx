...
/**
 * Stage names come from `STAGE_LABEL`, the map the stepper renders from.
 *
 * The last bullet said "**Journal**" — the enum key, not the label. The stage is called
 * **Reflect** everywhere a user can see it, and this is the third surface found naming it
 * by its key (after the dispatch-failure notifications). Interpolating removes the class.
 */
const PROJECTS_NOTE = `### The pipeline

- **${STAGE_LABEL.exploration}** — describe the idea, attach context, let agents research
- **${STAGE_LABEL.spec}** — AI-guided requirements with Q&A per section
- **${STAGE_LABEL.plan}** — task breakdown with audit before execution
- **${STAGE_LABEL.execute}** — agents build on a dedicated branch per project
- **${STAGE_LABEL.review}** — human review of every commit
- **${STAGE_LABEL.journal}** — capture what the team learned

### Dashboard metrics

- **Waiting for human** — a spec component you have not approved yet
- **Agents running** — live agent work across projects
- **Audit issues** — a stage whose latest audit came back with changes to make`;
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col">
        {projects.length + archived.length > 0 ? (
          ...
          <EmptyState
            ...
            // Named the real stages. This read "exploration, spec, freeze, and an
            // autonomous build" — "freeze" is not a stage (it is a permissions concept
            // in `stage-freeze.ts`), and plan and review were missing entirely.
            description={`Create your first project to start the flow — Forge takes it from idea through ${STAGE_KIND.map((k) => STAGE_LABEL[k]).join(" → ")}.`}
          />
        )}
      </CardContent>
    </Card>
    ...
    <PageFrame title="Projects" actions={newProject} width="full" fill>
      <StageShell
        metrics={[
          ...
          { label: 'Audit issues', value: metrics.auditIssues, muted: metrics.auditIssues === 0, sublabel: 'Stages needing a fix', icon: <AlertTriangle />, iconTint: 'rose' },
        ]}
        note={railNote}
      >
        {primary}
